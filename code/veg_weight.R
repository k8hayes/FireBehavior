...
ggplot(veg_weight, aes(y = DIFF, fill = SITE)) + geom_boxplot() + 
  labs(y = "dry weight / wet weight", title = "Difference in fuel moisture between sites")
...
veg_weight %>%
  ...
  labs(y = "dry weight / wet weight", x = "Plot ID", title = "Diff. in fuel moisture across plots")
