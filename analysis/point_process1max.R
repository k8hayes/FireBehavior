...
## Points #########################
  # mean number of trees per plot
    ...
    round(mean(meantree1$n_landscape)) # 363200
...
## Marks ####################
  ...
  ### Species ###############################
    ...
    meantreespp1 <- dbh %>% 
      ...
      summarise(n = n()) %>% # counts number of spp within plot
        ...
        summarise(AV = round(max(n))) # average of count
 ...
 ### Live / Dead #############################
  ...
  loc <- rbind(pime_loc, bene_loc, potr_loc, salix_loc, alcr_loc)
   ...
   # what's the average height per species within 1x burns
    height <- dbh %>% 
      ...
      summarise(MAX = max(HEIGHT_M), SD = sd(HEIGHT_M)) %>%
    ...
    height$MAX[1] <- height$MAX[2]
    height$SD[1] <- height$SD[2]
    
    height$MAX[7] <- height$MAX[8]
  ...
  bene_height <- height %>%
    filter(SPP == "BENE")
  bene_loc$HEIGHT_M[bene_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                              a = 0, mean = bene_height$MAX[bene_height$LIVE_DEAD=="DEAD"],
                                                              sd = bene_height$SD[bene_height$LIVE_DEAD=="DEAD"])
  bene_loc$HEIGHT_M[bene_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                              a = 0, mean = bene_height$MAX[bene_height$LIVE_DEAD=="LIVE"],
  ...
  potr_height <- height %>%
    filter(SPP == "POTR")
  potr_loc$HEIGHT_M[potr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = potr_height$MAX[potr_height$LIVE_DEAD=="DEAD"],
                                                          sd = potr_height$SD[potr_height$LIVE_DEAD=="DEAD"])
  potr_loc$HEIGHT_M[potr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = potr_height$MAX[potr_height$LIVE_DEAD=="LIVE"],
  ...
  pime_height <- height %>%
    filter(SPP == "PIME")
  pime_loc$HEIGHT_M[pime_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = pime_height$MAX[pime_height$LIVE_DEAD=="DEAD"],
                                                          sd = pime_height$SD[pime_height$LIVE_DEAD=="DEAD"])
  pime_loc$HEIGHT_M[pime_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = pime_height$MAX[pime_height$LIVE_DEAD=="LIVE"],
  ...
  alcr_height <- height %>%
    filter(SPP == "ALCR")
  alcr_loc$HEIGHT_M[alcr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = alcr_height$MAX[alcr_height$LIVE_DEAD=="DEAD"],
                                                          sd = alcr_height$SD[alcr_height$LIVE_DEAD=="DEAD"])
  alcr_loc$HEIGHT_M[alcr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = alcr_height$MAX[alcr_height$LIVE_DEAD=="LIVE"],
...
##### Willow 
  ...
  salix_height <- height %>%
    filter(SPP == "SALIX")
  salix_loc$HEIGHT_M[salix_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = salix_height$MAX[salix_height$LIVE_DEAD=="DEAD"],
                                                          sd = salix_height$SD[salix_height$LIVE_DEAD=="DEAD"])
  salix_loc$HEIGHT_M[salix_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = salix_height$MAX[salix_height$LIVE_DEAD=="LIVE"],
  ...
  live <- loc %>%
    filter(LIVE_DEAD == 1)
  quantile(live$HEIGHT_M, .9) # 12.85 meters
...
### Biomass #########################
  ...
  dbh_assign <- dbh %>%
    ...
    group_by(SPP, LIVE_DEAD) %>%
    summarise(MAX = max(DBH_CM), SD = sd(DBH_CM)) %>%
    complete(expand(dbh,LIVE_DEAD)) %>%
    filter(LIVE_DEAD != "NA")
  
  dbh_assign$MAX[1] <- dbh_assign$MAX[2]
  dbh_assign$SD[1] <- dbh_assign$SD[2]
  
  dbh_assign$MAX[7] <- dbh_assign$MAX[8]
  ...
  bene_dbh <- dbh_assign %>%
    filter(SPP == "BENE")
  bene_loc$DBH_CM[bene_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = bene_dbh$MAX[bene_dbh$LIVE_DEAD=="DEAD"],
                                                          sd = bene_dbh$SD[bene_dbh$LIVE_DEAD=="DEAD"])
  bene_loc$DBH_CM[bene_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = bene_dbh$MAX[bene_dbh$LIVE_DEAD=="LIVE"],
  ...
  potr_dbh <- dbh_assign %>%
    filter(SPP == "POTR")
  potr_loc$DBH_CM[potr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = potr_dbh$MAX[potr_dbh$LIVE_DEAD=="DEAD"],
                                                          sd = potr_dbh$SD[potr_dbh$LIVE_DEAD=="DEAD"])
  potr_loc$DBH_CM[potr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = potr_dbh$MAX[potr_dbh$LIVE_DEAD=="LIVE"],
  ...
  pime_dbh <- dbh_assign %>%
    filter(SPP == "PIME")
  pime_loc$DBH_CM[pime_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = pime_dbh$MAX[pime_dbh$LIVE_DEAD=="DEAD"],
                                                          sd = pime_dbh$SD[pime_dbh$LIVE_DEAD=="DEAD"])
  pime_loc$DBH_CM[pime_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = pime_dbh$MAX[pime_dbh$LIVE_DEAD=="LIVE"],
  ...
  alcr_loc$DBH_CM[alcr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                          a = 0, mean = alcr_dbh$MAX[alcr_dbh$LIVE_DEAD=="DEAD"],
                                                          sd = alcr_dbh$SD[alcr_dbh$LIVE_DEAD=="DEAD"])
  alcr_loc$DBH_CM[alcr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                          a = 0, mean = alcr_dbh$MAX[alcr_dbh$LIVE_DEAD=="LIVE"],
  ...
  salix_dbh <- dbh_assign %>%
    filter(SPP == "SALIX")
  salix_loc$DBH_CM[salix_loc$LIVE_DEAD== 0] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                            a = 0, mean = salix_dbh$MAX[salix_dbh$LIVE_DEAD=="DEAD"],
                                                            sd = salix_dbh$SD[salix_dbh$LIVE_DEAD=="DEAD"])
  salix_loc$DBH_CM[salix_loc$LIVE_DEAD== 1] <- rtruncnorm(round(max(meantree1$n_landscape)), 
                                                            a = 0, mean = salix_dbh$MAX[salix_dbh$LIVE_DEAD=="LIVE"],
  ...
  write.csv(trees_WFDS, here("data/output/max/tree_WFDS_1x_max.csv"), row.names = FALSE)
