...
# scaling to landscape level
  meantree1$n_m2 <- round(meantree1$n / meantree1$m2) 

  meantree1$n_plot <- round(meantree1$n_m2 * 400)
  
  round(mean(meantree1$n_plot)) # 4187
  
  meantree1$n_landscape <- round(meantree1$n_m2 * 2400) #25120
  ...
  height$AV[1] <- height$AV[2] # alcr
  height$SD[1] <- height$SD[2]
  
  height$SD[3] <- height$SD[4] # arcto # only 1 dead, so no sd
  
  height$AV[9] <- height$AV[10] # potr
  ...
  sub_assign$AV[1] <- sub_assign$AV[2]
  sub_assign$SD[1] <- sub_assign$SD[2]
  ...
  bene_dbh <- sub_assign %>%
    filter(SPP == "BENE")
  bene_loc$BD[bene_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                        a = 0, mean = bene_dbh$AV[bene_dbh$LIVE_DEAD=="DEAD"],
                                                        sd = bene_dbh$SD[bene_dbh$LIVE_DEAD=="DEAD"])
  bene_loc$BD[bene_loc$LIVE_DEAD== 1] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
  ...
  potr_dbh <- sub_assign %>%
    filter(SPP == "POTR")
  potr_loc$BD[potr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                        a = 0, mean = potr_dbh$AV[potr_dbh$LIVE_DEAD=="DEAD"],
                                                        sd = potr_dbh$SD[potr_dbh$LIVE_DEAD=="DEAD"])
  potr_loc$BD[potr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
  ...
  pime_dbh <- sub_assign %>%
    filter(SPP == "PIME")
  pime_loc$BD[pime_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                        a = 0, mean = pime_dbh$AV[pime_dbh$LIVE_DEAD=="DEAD"],
                                                        sd = pime_dbh$SD[pime_dbh$LIVE_DEAD=="DEAD"])
  pime_loc$BD[pime_loc$LIVE_DEAD== 1] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
  ...
  alcr_dbh <- sub_assign %>%
    filter(SPP == "ALCR")
  alcr_loc$BD[alcr_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                        a = 0, mean = alcr_dbh$AV[alcr_dbh$LIVE_DEAD=="DEAD"],
                                                        sd = alcr_dbh$SD[alcr_dbh$LIVE_DEAD=="DEAD"])
  alcr_loc$BD[alcr_loc$LIVE_DEAD== 1] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                        a = 0, mean = alcr_dbh$AV[alcr_dbh$LIVE_DEAD=="LIVE"],
                                                        sd = alcr_dbh$SD[alcr_dbh$LIVE_DEAD=="LIVE"])
  
  # loc$BD[loc$SPP=="ARCTO"] <- rtruncnorm(round(mean(meantree1$n)), a = 0,mean = dbh_assign$AV[dbh_assign$SPP == "ARCTO"],
  ...
  salix_dbh <- sub_assign %>%
    filter(SPP == "SALIX")
  salix_loc$BD[salix_loc$LIVE_DEAD== 0] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
                                                          a = 0, mean = salix_dbh$AV[salix_dbh$LIVE_DEAD=="DEAD"],
                                                          sd = salix_dbh$SD[salix_dbh$LIVE_DEAD=="DEAD"])
  salix_loc$BD[salix_loc$LIVE_DEAD== 1] <- rtruncnorm(round(mean(meantree1$n_landscape)), 
