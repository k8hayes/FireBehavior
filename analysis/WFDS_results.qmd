...
meanmod <- wind_sum %>%
  ...
  labs(x = 'X, distance from transition (m)', y = 'Wind speed (m/s)',
       title = "Mean Fuel x Moderate Weather") + theme_half_open() + 
    background_grid() + 
...
meanhi <- wind_sum %>%
  ...
  geom_ribbon(aes(x, ymin = low, ymax = high, fill = burn), alpha = 0.2) + 
  labs(x = 'X, distance from transition (m)', y = 'Wind speed (m/s)',
       title = "Mean Fuel x Extreme Weather") + theme_half_open() + 
    background_grid() + 
  scale_color_manual(values = c("#74c476", "#fd8d3c")) + 
  scale_fill_manual(values = c("#74c476", "#fd8d3c")) +
  theme(legend.position = "none")  + 
...
himod <- wind_sum %>%
  ...
  geom_ribbon(aes(x, ymin = low, ymax = high, fill = burn), alpha = 0.2) + 
  labs(x = 'X, distance from transition (m)', y = 'Wind speed (m/s)',
       title = "High Fuel x Moderate Weather") + theme_half_open() + 
    background_grid() + 
  scale_color_manual(values = c("#74c476", "#fd8d3c")) + 
  scale_fill_manual(values = c("#74c476", "#fd8d3c")) +
  theme(legend.position = "none") + 
...
highhigh <- wind_sum %>%
  ...
  geom_ribbon(aes(x, ymin = low, ymax = high, fill = burn), alpha = 0.2) + 
  labs(x = 'X, distance from transition (m)', y = 'Wind speed (m/s)',
       title = "High Fuel x Extreme Weather") + theme_half_open() + 
    background_grid() + 
  scale_color_manual(values = c("#74c476", "#fd8d3c")) + 
  scale_fill_manual(values = c("#74c476", "#fd8d3c")) +
  theme(legend.position = "none") + 
...
```{r}
ros <- dplyr::select(results, sim, ros) %>% 
  unnest() %>%
  mutate(burn = ifelse(sim == "1x_meanfuel_hiwx", "1x", 
                       ifelse(sim == "1x_meanfuel_modwx", "1x",
                              ifelse(sim == "1x_hifuel_hiwx", "1x",
...
meanmod <- ros %>%
  filter(sim == "1x_meanfuel_modwx" | sim == "3x_meanfuel_modwx") %>%
  ggplot() + 
  geom_line(aes(x = x, y = ros, col = burn)) +
  labs(x = 'X, distance from transition (m)', y = 'Rate of Spread (m/s)',
       title = "Mean Fuel x Moderate Weather") + theme_half_open() + 
    background_grid() + 
  scale_color_manual(values = c("#253494", "#fec44f")) +
  theme(legend.position = "none") +
  ylim(c(0,2))
...
ggsave2(plot = meanmod,
        filename = "meanmod.png",
        path = "/Users/katherinehayes/Google Drive/Projects/Reburns/Chapters/Flammabllity/output/Figures/Fig8_ROS/",
        width = 6, height = 5, units = "in")
