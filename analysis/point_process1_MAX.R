...
## Points #########################
  # mean number of trees per plot
    ...
    round(quantile(meantree1$n, .9)) #397
    ...
    round(quantile(meantree1$n_landscape, .9)) # 672960
    
  # generating point cloud  
  x <- round(runif(round(quantile(meantree1$n_landscape, .9)), 0, 800))
  y <- round(runif(round(quantile(meantree1$n_landscape, .9)), 0, 300))
...
### Biomass #########################
  ...
  # )
  # 
  # write.csv(trees_WFDS, here("data/output/tree1_WFDS_MAX.csv"), row.names = FALSE)
  # 
